    ...
    override class func spec() {
        ...
        describe("Fetch Pokemon") {
            ...
            it("should return with ListPokemon") {
                ...
                let _ = homeUseCase.fetchPokemons().sink { _ in
                    
                } receiveValue: { pokemonList in
                    expect { pokemonList.count }.to(equal(expectedCount))
                    expect { pokemonList[0].name }.to(equal(expectedValue.name))
                    expect { pokemonList[0].image }.to(equal(expectedValue.image))
                    expect { pokemonList[0].color }.to(equal(expectedValue.color))
                }
            }
